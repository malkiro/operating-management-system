function getDataFromObList(model, path) {
    var parts = path.split('.');
    if (parts.length > 1 && typeof model[parts[0]] === 'object') {
        return getDataFromObList(model[parts[0]], parts.splice(1).join('.'));
    } else {
...
function toDecimal(number,dpoint=2) {
...
function table(parentid,id,metadata, hasBtn=true){
    ...
    // row1.border="0";
    row1.classList.add('table-primary');
...
function fillTable(tableid,obs,modifyFunction=null,deletefunction=null,printfunction=null){
    ...
    for(var i=0; i<obs.length; i++){
        ...
        if(hasButton){
            ...
            buttonup.setAttribute('data-toggle',"modal");
            buttonup.setAttribute('data-target',"#formmodal");
            ...
            buttonvie.setAttribute('data-target',"#dataviewModal");
...
function fillCombo2(combo, message, list, attribute, selectedvaluelist) {
    combo.innerHTML = "";
    if (message != "") {
        var prompt = document.createElement("option");
        prompt.value = "";
    ...
    for (index in list) {
        ...
        for(ind in selectedvaluelist){
            if(list[index][attribute] == selectedvaluelist[ind][attribute])
                option.selected = "selected";
            combo.style.border = valid;
        }
...
function fillCombo3(combo, message, list, attribute,attribute2, selectedvalue) {
    ...
    for (index in list) {
        var option = document.createElement("option");
        option.value = JSON.stringify(list[index]);
        var text = document.createTextNode(list[index][attribute] +" --> "+getDataFromObList(list[index],attribute2) +"" );
        if (list[index][attribute] == selectedvalue) {
            option.selected = "selected";
            combo.style.border = valid;
        }
...
function fieldsetBinder(fieldsetid,field,pattern,obj,prop,oldobj){
    field.setAttribute('data-pattern',pattern);
    field.setAttribute('data-object',obj);
    field.setAttribute('data-oldobject',oldobj);
    ...
    if (regpattern.test(val)) {
        ob[prop] = val;
        if (oldob != null && oldob[prop] != ob[prop]){
            fieldsetid.style.border = updated;
        }else{
            fieldsetid.style.border = valid;
        }
    }
    else {
        fieldsetid.style.border = invalid;
        ob[prop] = null;
    }
}

function spantextFieldBinder(field,help,pattern,obj,prop,oldobj){
    field.setAttribute('data-pattern',pattern);
    field.setAttribute('data-object',obj);
    field.setAttribute('data-oldobject',oldobj);
    ...
    if (regpattern.test(val)) {
        ...
        if (oldob != null && oldob[prop] != ob[prop]){
            field.style.border = updated;
            help.style.borderRight = updated;
            help.style.borderTop = updated;
            help.style.borderBottom = updated;
        }else{
            field.style.borderLeft = valid;
            field.style.borderTop = valid;
            field.style.borderBottom = valid;
            help.style.borderRight = valid;
            help.style.borderTop = valid;
            help.style.borderBottom = valid;
        }
    }
    else {
        field.style.borderLeft = invalid;
        field.style.borderTop = invalid;
        field.style.borderBottom = invalid;
        help.style.borderRight = invalid;
        help.style.borderTop = invalid;
        help.style.borderBottom = invalid;
        ob[prop] = null;
    }
}
...
function comboBox2Binder(field,pattern,obj,prop,oldobj){
    ...
    if (oldob != null && oldob[prop].id != ob[prop].id){
        ...
        field.parentNode.children[1].style.border = updated;
    } else {
        // $('.select2-container').css('border', valid);
        field.parentNode.children[1].style.border = valid;
...
function fileChooser(parent,id,lblText,labelWidth,extenstions,maxsize,required,ob,prop,oldob){
    ...
    // buttonArea.style.border="1px solid black";
    buttonArea.style.width="500px";
    ...
    fieldButton.style.width = "370px";
    fieldButton.classList.add('btn');
    fieldButton.classList.add('btn-sm');
    fieldButton.classList.add('btn-outline-info');
    ...
    fieldButtonC.style.width = "300px";
        ...
        if(size>maxsize){
            ...
            if(match){
                ...
                reader.onload = function(e) {
                    ...
                    fileBox.style.height = "170px";
                    fileBox.style.width = "180px";
                    fileBox.style.border = "0px";
                    fileBox.style.margin = "-10px 10px 10px 20px";
...
function tableForInnerForm(parentId, id, metadata, hasBtn = true) {
    ...
    table.classList.add('table-bordered');
    table.style.backgroundImage='radial-gradient(circle,white,lightcyan,lightcyan,lightskyblue)';
...
function fillInnerTable(tableId,obs, modifyFunction = null, deleteFunction = null) {
    ...
    for (var i = 0; i < obs.length; i++) {
        ...
        if (hasButton) {
            ...
            buttonInnerUpdate.innerHTML = '<i class="fa fa-edit"></i> Edit';
            ...
            buttonInnerDelete.innerHTML = '<i class="fa fa-trash-alt"></i> Remove';
