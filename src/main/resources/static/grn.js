...
function initialize() {
    ...
    txtMprice.addEventListener("change",txtMpriceKU);
    txtSprice.addEventListener("change",txtSpriceKU);
...
function selectDeleteRow() {
    for(index in grns){
        ...
        // if (grns[index].grnstatusId.name == "Completed") {
        //     tblGrn.children[1].children[index].lastChild.children[0].disabled = true;
...
function loadForm() {
    ...
    var thirtydaysafterday = new Date();
    thirtydaysafterday.setDate(today.getDate()+30);
    txtExpdate.min = thirtydaysafterday.getFullYear()+"-"+getmonthdate(thirtydaysafterday)
    txtExpdate.max = thousanddatsafterday.getFullYear()+"-"+getmonthdate(thousanddatsafterday);

    //Manufacture Date
    var twoweekbeforeday = new Date();
    twoweekbeforeday.setDate(today.getDate()-60);
...
function txtMpriceKU() {
    if (isNaN(parseFloat(txtMprice.value))){
        txtMprice.value = "";
        txtMprice.style.border = invalid;
        swal({
            title: "Market Price is not allowed",
            text: "\n",
            icon: "warning",
            buttons: true,
        })
    }else if (parseFloat(txtMprice.value) < 0) {
        txtMprice.value = "";
        txtMprice.style.border = invalid;
        swal({
            title: "Market Price can't be a negative",
            text: "\n",
            icon: "warning",
            buttons: true,
        })
...
function txtSpriceKU() {
    if (isNaN(parseFloat(txtSprice.value))){
        txtSprice.value = "";
        txtSprice.style.border = invalid;
        swal({
            title: "Real Price is not allowed",
            text: "\n",
            icon: "warning",
            buttons: true,
        })
    }else if (parseFloat(txtSprice.value) < 0) {
        txtSprice.value = "";
        txtSprice.style.border = invalid;
        swal({
            title: "Real Price can't be a negative",
            text: "\n",
            icon: "warning",
            buttons: true,
        })
...
function btnDeleteMC(gr) {
    ...
    swal({
        ...
        text :  "\nGRN Number : " + grn.grnno +
            ...
            "\nOrder Date : " + grn.addeddate +
            "\nOrdered By : " + grn.employeeId.callingname,
        icon: "resourse/image/grndel.png",
