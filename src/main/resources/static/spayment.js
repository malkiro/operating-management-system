...
function selectDeleteRow() {
    for(index in spayments){
            ...
            tblSpayment.children[1].children[index].lastChild.children[2].style.marginLeft = "60px";
    }
}
...
function txtPaidamountMC() {
    if (isNaN(parseFloat(txtPaidamount.value))){
        txtPaidamount.value = "";
        txtPaidamount.style.border = invalid;
        swal({
            title: "Paid Amount is not allowed",
            text: "\n",
            icon: "warning",
            buttons: true,
        })
    }else {
        if (parseFloat(txtPaidamount.value) < 0) {
            txtPaidamount.value = "";
            txtPaidamount.style.border = invalid;
            swal({
                title: "Paid Amount can't be a negative number",
                text: "\n",
                icon: "warning",
                buttons: true,
            })
        } else if (parseFloat(txtPaidamount.value) == 0) {
            txtPaidamount.value = "";
            txtPaidamount.style.border = invalid;
            swal({
                title: "Paid Amount is not allowed",
...
function savedata() {
    swal({
        ...
        icon: "resourse/image/spayment.png",
        buttons: [
            'No, Cancel',
            'Yes, Save!'
        ],
...
function btnDeleteMC(spay) {
    ...
    swal({
        ...
        text :  "\nBill Number : " + spayment.sbillno +
            ...
            "\nPaid Date  : " + spayment.date +
            "\nPaid By : " + spayment.employeeId.callingname,
        icon: "resourse/image/spaymentdel.png",
