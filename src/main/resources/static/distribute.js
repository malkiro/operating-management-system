...
function initialize() {
    ...
    cmbRoute.addEventListener("change", cmbRouteMC);
    dteDeldate.addEventListener("change", dteDeldateMC);
...
function selectDeleteRow() {
    for(index in distributes){
        ...
        if (distributes[index].distributionstatusId.name == "Delivered") {
            tblDistribute.children[1].children[index].lastChild.children[0].disabled = true;
            tblDistribute.children[1].children[index].lastChild.children[0].style.cursor = "not-allowed";
            tblDistribute.children[1].children[index].lastChild.children[1].disabled = true;
            tblDistribute.children[1].children[index].lastChild.children[1].style.cursor = "not-allowed";
        }
    }
...
function loadForm() {
    ...
    fillCombo3(cmbVehicle, "Select Vehicle", vehicles, "vehiclename","vehicletypeId.name");
    ...
    dteDeldate.min = today.getFullYear() + "-" + month + "-" + date;
    dteDeldate.max = twoweekafterday.getFullYear()+"-"+getmonthdate(twoweekafterday);
    ...
    cmbInvoice.setAttribute("disabled", "disabled");
    cmbEmployeecrew.setAttribute("disabled", "disabled");
    cmbVehicle.setAttribute("disabled", "disabled");
    cmbAgent.setAttribute("disabled", "disabled");
    cmbDriver.setAttribute("disabled", "disabled");
    $("#chkClose").prop("checked", false);

    txtTotalinvoice.value = "";
    txtTotalinvoice.disabled = false;
    dteDeldate.value = "";
...
function refreshInvoiceInnerForm() {
    ...
    if(distribute.distributeInvoiceList.length != 0){
        ...
        console.log(distribute.distributeInvoiceList.length)
        txtTotalinvoice.value = distribute.distributeInvoiceList.length;
        distribute.totalinvoice = txtTotalinvoice.value;

        if(olddistribute != null && olddistribute.totalinvoice != distribute.totalinvoice){
            txtTotalinvoice.style.border = updated;
        }else {
            txtTotalinvoice.style.border = valid;
            txtTotalinvoice.disabled = true;
        }
    }else  distribute.totalinvoice = null;
...
function cmbRouteMC() {
//get invoice list by given route
    invoices = httpRequest("../invoice/listbyroute?routeid="+JSON.parse(cmbRoute.value).id,"GET");
    fillCombo(cmbInvoice, "Select Invoice", invoices, "invoiceno");
    cmbInvoice.removeAttribute("disabled", "disabled");
}

function dteDeldateMC() {
    cmbEmployeecrew.removeAttribute("disabled", "disabled");
    cmbVehicle.removeAttribute("disabled", "disabled");
    cmbAgent.removeAttribute("disabled", "disabled");
    cmbDriver.removeAttribute("disabled", "disabled");

    var stdate = new Date(dteDeldate.value);
    // edate.setDate(stdate.getDate()+ parseInt(txtDaysNo.value));
    dteDeldate.value = stdate.getFullYear() + "-"+getmonthdate(stdate);

    // var availableguide = httpRequest("/employee/listbyavilableguide?startdate="+dteStart.value+"&enddate="+dteEnd.value,"GET");
    // fillCombo3(cmbGuideID,"Select a Guide",availableguide,"number","callingname");

    var availabledeliverysupportivecrew = httpRequest("/employee/listbyavilablesuportivecrow?distributiondate="+dteDeldate.value,"GET");
    fillCombo(cmbEmployeecrew, "Select Employee", availabledeliverysupportivecrew, "callingname");

    var availabledeliveryagent = httpRequest("/employee/listbyavilabledeliveryagent?distributiondate="+dteDeldate.value,"GET");
    fillCombo(cmbAgent, "Select Delivery Agent", availabledeliveryagent, "callingname");

    var availabledriver = httpRequest("/employee/listbyavilabledriver?distributiondate="+dteDeldate.value,"GET");
    fillCombo(cmbDriver, "Select Driver", availabledriver, "callingname");

    var availablevehicle = httpRequest("/vehicle/listbyavilablevehicle?distributiondate="+dteDeldate.value,"GET");
    fillCombo3(cmbVehicle, "Select Vehicle", availablevehicle, "vehiclename","vehicletypeId.name");

    distribute.distributiondate=dteDeldate.value;
}
...
function savedata() {
    ...
    swal({
        ...
        icon: "resourse/image/distributeadd.png",
        buttons: [
            'No, Cancel',
            'Yes, Save!'
        ],
...
function fillForm(dis,rowno){
    ...
    fillCombo3(cmbVehicle, "Select Vehicle", vehicles, "vehiclename","vehicletypeId.name",distribute.vehicleId.vehiclename);
    ...
    if (errors == "") {
        ...
        if (updates == "")
            ...
            swal({
                ...
                text: "\n"+ getUpdates(),
                icon: "resourse/image/distributeup.png",
...
function btnDeleteMC(dis) {
    ...
    swal({
        ...
        text :  "\nDelivery No : " + distribute.distributionno +
            ...
            "\nCreated Date : " + distribute.addeddate +
            "\nCreated By : " + distribute.employeeId.callingname,
        icon: "resourse/image/distributedel.png",
        closeOnClickOutside: false,
