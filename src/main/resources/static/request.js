...
function initialize() {
    ...
    employeeconfirmed = httpRequest("../employee/confirmlist","GET");
...
function selectDeleteRow() {
    for(index in requests){
        ...
        if (requests[index].requeststatusId.name == "Completed") {
            tblRequest.children[1].children[index].lastChild.children[0].disabled = true;
            tblRequest.children[1].children[index].lastChild.children[0].style.cursor = "not-allowed";
            tblRequest.children[1].children[index].lastChild.children[1].disabled = true;
            tblRequest.children[1].children[index].lastChild.children[1].style.cursor = "not-allowed";
        }
    }
...
function savedata() {
    ...
    swal({
        ...
        icon: "resourse/image/requestformadd.png",
        buttons: [
            'No, Cancel',
            'Yes, Save!'
        ],
...
function btnUpdateMC() {
    ...
    if (errors == "") {
        var updates = getUpdates();
            swal({
                title: "Are you sure to confirm following Request details...?",
                text: "\n"+ getUpdates() +
                    "\nRequest Number : " + request.no +
                    "\nRequest Status : " + request.requeststatusId.name +
                    "\nRequested Date  : " + request.requesteddate +
                ...
                icon: "resourse/image/requestformup.png",
...
function btnDeleteMC(req) {
    ...
    swal({
        ...
        text :  "\nRequest Number : " + request.no +
            ...
            "\nRequested By  : " + request.employeeId.callingname +
            "\nRequested Detail  : " + request.requestdetails,
        icon: "resourse/image/requestformdel.png",
