...
function initialize() {
    ...
    txtRecievedAmount.addEventListener("change", txtRecievedAmountKU);
...
function selectDeleteRow() {
    for(index in cpayments){
        if(cpayments[index].cpaymentstatusId.name =="Deleted"){
            tblCpayment.children[1].children[index].style.color = "#f00";
            tblCpayment.children[1].children[index].style.fontWeight = "bold";
            tblCpayment.children[1].children[index].lastChild.children[1].disabled = true;
            tblCpayment.children[1].children[index].lastChild.children[1].style.cursor ="not-allowed";
        }
    }
}
...
function viewitem(cpay,rowno) {
    ...
    fillInnerTable("tblPrintInnerIvoice", cpayment.cpaymentInvoiceList, mdifyInvoiceInnerForm , deleteInvoiceInnerForm);
